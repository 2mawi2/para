...
kill_cursor_if_possible() {
  WORKTREE_DIR="$1"
  STORED_PID="$2"  # Optional PID parameter
  
  # Strategy 0: Use stored PID from parameter (most reliable)
  if [ -n "$STORED_PID" ] && [ "$STORED_PID" != "" ]; then
    if kill -0 "$STORED_PID" 2>/dev/null; then
      echo "  → closing Cursor PID $STORED_PID (from state file)"
      kill "$STORED_PID" 2>/dev/null && echo "    ✓ closed successfully" || true
      return 0
    else
      echo "  → stored PID $STORED_PID no longer running"
    fi
  fi
  
  # Strategy 1: Use lsof to find processes with files open in the worktree
  if command -v lsof >/dev/null 2>&1; then
    # Find PIDs of processes with files open in the worktree directory
    PIDS=$(lsof +D "$WORKTREE_DIR" 2>/dev/null | awk 'NR>1 && $1=="'"$CURSOR_CMD"'" {print $2}' | sort -u)
    if [ -n "$PIDS" ]; then
      echo "  → closing Cursor instance using worktree $WORKTREE_DIR"
      for pid in $PIDS; do
        kill "$pid" 2>/dev/null && echo "    ✓ closed PID $pid" || true
      done
      return 0
    fi
  fi
  
  # Strategy 2: Try Cursor CLI to close specific workspace (if supported)
  if command -v "$CURSOR_CMD" >/dev/null 2>&1; then
    # Some editors support closing specific workspaces
    "$CURSOR_CMD" --help 2>/dev/null | grep -q "close-workspace" && \
    "$CURSOR_CMD" --close-workspace "$WORKTREE_DIR" 2>/dev/null && \
    echo "  → closed Cursor workspace for $WORKTREE_DIR" && return 0
  fi
  
  # Strategy 3: Enhanced process matching (safer than original)
  if command -v pgrep >/dev/null 2>&1 && command -v ps >/dev/null 2>&1; then
    # Find Cursor processes and check their command line for our specific path
    PIDS=$(pgrep -f "$CURSOR_CMD" 2>/dev/null)
    if [ -n "$PIDS" ]; then
      for pid in $PIDS; do
        # Check if this specific PID has our worktree path in its command line
        if ps -p "$pid" -o args= 2>/dev/null | grep -q "$WORKTREE_DIR"; then
          echo "  → closing Cursor PID $pid (matched worktree path)"
  ...
  # Strategy 4: Fallback - original approach but safer
  if command -v pkill >/dev/null 2>&1; then
    # More precise pattern - only if we can confirm it's our specific path
    PATTERN="${CURSOR_CMD}.*$(basename "$WORKTREE_DIR")"
    if pkill -f "$PATTERN" 2>/dev/null; then
      echo "  → closed Cursor instance (pattern match fallback)"
      return 0
    fi
  fi
  
  # If we get here, no Cursor instances were found/closed
  echo "  → no Cursor instances found for $WORKTREE_DIR"
  return 1
}
...
list_sessions() {
  ...
  for state_file in "$STATE_DIR"/*.state; do
    ...
    IFS='|' read -r TEMP_BRANCH WORKTREE_DIR BASE_BRANCH CURSOR_PID < "$state_file"
...
get_session_info() {
  ...
  # Read state file - handle both old (3 fields) and new (4 fields) formats
  IFS='|' read -r TEMP_BRANCH WORKTREE_DIR BASE_BRANCH CURSOR_PID < "$STATE_FILE"
  # CURSOR_PID may be empty for old state files or when Cursor wasn't launched
}
...
continue_merge() {
  ...
  kill_cursor_if_possible "$WORKTREE_DIR" "$CURSOR_PID"
...
clean_all_sessions() {
  ...
  for state_file in "$STATE_DIR"/*.state; do
    ...
    # Kill associated Cursor processes
    kill_cursor_if_possible "$WORKTREE_DIR" "$CURSOR_PID"
...
if [ "$#" -eq 0 ]; then
  ...
  if command -v "$CURSOR_CMD" >/dev/null 2>&1; then
    ...
    echo "$TEMP_BRANCH|$WORKTREE_DIR|$BASE_BRANCH|$CURSOR_PID" > "$STATE_DIR/$SESSION_ID.state"
  else
