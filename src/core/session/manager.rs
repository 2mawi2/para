...
impl SessionManager {
    ...
    pub fn create_session_with_all_options(
        ...
        let parent_branch = git_service
            ...
            .get_current_branch()
            .unwrap_or_else(|_| {
                // Fall back to base_branch or main if we can't get current branch
                base_branch.unwrap_or_else(|| {
                    git_service
                        .repository()
                        .get_main_branch()
...
mod tests {
    ...
    #[test]
    fn test_parent_branch_tracking() {
        let temp_dir = TempDir::new().unwrap();
        ...
        manager.save_state(&session).unwrap();
        let loaded = manager.load_state("feature-test").unwrap();

        assert_eq!(loaded.parent_branch, Some("develop".to_string()));
        assert_eq!(loaded.name, "feature-test");
        assert_eq!(loaded.branch, "para/feature-test");
    }

    #[test]
    fn test_parent_branch_backwards_compatibility() {
        let temp_dir = TempDir::new().unwrap();
        ...
        // Create session without parent branch (old format)
        let session = SessionState::new(
            "old-format".to_string(),
            "para/old-format".to_string(),
            temp_dir.path().join("worktree"),
        );

        // Save and reload
        manager.save_state(&session).unwrap();
        let loaded = manager.load_state("old-format").unwrap();
