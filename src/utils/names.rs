...
pub fn generate_friendly_name() -> Result<String> {
    ...
    let adjective = ADJECTIVES.choose(&mut rng).ok_or_else(|| {
        ParaError::config_error("Name generation failed: adjectives list empty".to_string())
    })?;
    let noun = NOUNS.choose(&mut rng).ok_or_else(|| {
        ParaError::config_error("Name generation failed: nouns list empty".to_string())
    })?;
