...
use crate::platform::launch_file_parser::parse_ide_from_launch_contents;
    ...
    fn determine_actual_ide(
        ...
        let actual_ide = if launch_file.exists() {
            ...
            if let Ok(contents) = std::fs::read_to_string(&launch_file) {
                // Use the shared launch file parser utility
                parse_ide_from_launch_contents(&contents, ide_name)
            } else {
                ide_name.to_string()
            }
        } else {
