...
impl<'a> BranchManager<'a> {
    ...
    pub fn get_tracking_branch(&self, branch: &str) -> Result<Option<String>> {
        let output =
            execute_git_command(self.repo, &["config", &format!("branch.{}.remote", branch)]);
        ...
        let remote = output?;
        let merge_ref =
            execute_git_command(self.repo, &["config", &format!("branch.{}.merge", branch)])?;
...
mod tests {
    ...
    fn test_clean_archived_branches() {
        ...
        manager
            .create_branch("cleanup-test", &initial_branch)
            .expect("Failed to create branch");
        repo.checkout_branch(&initial_branch)
            .expect("Failed to checkout initial branch");

        manager
            .move_to_archive("cleanup-test", "pc")
            .expect("Failed to archive branch");

        let archived_before = manager
            .list_archived_branches("pc")
            .expect("Failed to list archived branches");
        assert_eq!(archived_before.len(), 1);

        let cleaned_count = manager
            .clean_archived_branches("pc")
            .expect("Failed to clean archived branches");

        let archived_after = manager
            .list_archived_branches("pc")
            .expect("Failed to list archived branches");
