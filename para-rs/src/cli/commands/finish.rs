...
fn validate_branch_name(name: &str) -> Result<()> {
    ...
    if name.starts_with('-') || name.ends_with('-') {
        return Err(ParaError::invalid_args("Branch name cannot start or end with hyphen"));
    }
    
    if name.contains("..") || name.contains("//") {
        return Err(ParaError::invalid_args("Branch name contains invalid character sequence"));
    }
